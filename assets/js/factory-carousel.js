document.addEventListener('DOMContentLoaded',()=>{
  ...
  const render=()=>{
    ...
    dots.innerHTML=Array.from({length:maxIndex()+1},(_,i)=>`<button type="button" class="factory-carousel-dot ${i===index?'active':''}" data-factory-dot="${i}" aria-label="${i+1}번째"></button>`).join('');
    dots.querySelectorAll('[data-factory-dot]').forEach(btn=>btn.onclick=()=>{index=Number(btn.dataset.factoryDot);render();restart();});
  };
  const next=()=>{index=index>=maxIndex()?0:index+1;render();};
  const prev=()=>{index=index<=0?maxIndex():index-1;render();};
