...
function validateStep(n){if(n===1){captureDimensions();if(!state.product){alert('박스 형태를 선택해주세요.');return false}if(!state.qty||(state.qty==='직접입력'&&!state.customQty)){alert('제작 수량을 선택해주세요.');return false}if(!state.w||!state.d||!state.h){alert('완성 사이즈를 입력해주세요.');return false}}if(n===2){if(!state.paper){alert('종이·소재를 선택해주세요.');return false}if(state.paper.gsm.length&&!state.gsm){alert('평량을 선택해주세요.');return false}}if(n===3){if(!state.printMethod||!state.printColor||!state.printSide){alert('인쇄 방식, 색상, 인쇄 면을 모두 선택해주세요.');return false}}if(n===4&&!state.coating){alert('코팅 여부를 선택해주세요. 코팅이 없으면 “코팅 없음”을 선택해주세요.');return false}return true;}
function saveLocal(payload){const key='fineb_quote_requests';const list=JSON.parse(localStorage.getItem(key)||'[]');list.unshift(payload);localStorage.setItem(key,JSON.stringify(list.slice(0,200)));localStorage.setItem('fineb_quote_draft',JSON.stringify(payload));}
async function submitQuote(){const btn=$('#submitQuote');if(btn.dataset.busy==='1')return;const name=$('#name').value.trim(),phone=$('#phone').value.trim(),email=$('#email').value.trim();if(!name||!phone||!email){alert('담당자명, 연락처, 이메일을 입력해주세요.');return}if(!$('#privacyQuote')?.checked){alert('개인정보 수집 및 이용에 동의해주세요.');return}captureDimensions();const files=[...$('#fileInput').files];const payload={id:'Q-'+Date.now(),type:'quote',status:'신규',createdAtClient:new Date().toISOString(),company:$('#company').value.trim(),name,phone,email,message:$('#message').value.trim(),privacyAgreed:true,spec:{product:state.product?.name||'',category:state.product?.cat||'',qty:qtyLabel(),size:state.w&&state.d&&state.h?`${state.w} × ${state.d} × ${state.h} mm`:'',paper:state.paper?.name||'',gsm:state.gsm||'',printMethod:state.printMethod||'',printColor:state.printColor||'',printSide:state.printSide||'',coating:state.coating||'',finishes:[...state.finishes],insert:state.insert||''}};btn.dataset.busy='1';const old=btn.textContent;btn.textContent='접수 중...';btn.disabled=true;try{const fb=await firebasePromise;if(!fb)throw new Error('FIREBASE_MODULE');const saved=await fb.savePublicRequest('quote',payload,files);saveLocal({...saved,files:(saved.files||[]).map(f=>({name:f.name,size:f.size,url:f.url}))});alert('견적 요청이 정상 접수되었습니다.');}catch(err){saveLocal({...payload,files:files.map(f=>({name:f.name,size:f.size}))});console.warn('Firebase quote fallback:',err);alert('요청 내용을 브라우저에 임시 저장했습니다. Firebase 서비스 활성화 후에는 중앙 관리자에서 바로 확인됩니다.');}finally{btn.dataset.busy='0';btn.textContent=old;btn.disabled=false;}}
function init(){const qp=new URLSearchParams(location.search).get('cat');if(CATS.some(([id])=>id===qp))state.cat=qp;renderCats();renderProducts();renderQty();renderPapers();renderGsm();renderSingle('#printMethodGrid',PRINT_METHODS,'printMethod');renderSingle('#printColorGrid',PRINT_COLORS,'printColor');renderSingle('#printSideGrid',PRINT_SIDES,'printSide');renderSingle('#coatingGrid',COATINGS,'coating');renderFinish();renderSingle('#insertGrid',INSERTS,'insert');updateSummary();updateTrack();['width','depth','height'].forEach(id=>$('#'+id)?.addEventListener('input',updateSummary));$$('.next-step').forEach(btn=>btn.addEventListener('click',()=>{if(validateStep(state.step))showStep(Math.min(5,state.step+1));}));$$('.prev-step').forEach(btn=>btn.addEventListener('click',()=>showStep(Math.max(1,state.step-1)));$$('.track-item').forEach((btn,i)=>btn.addEventListener('click',()=>{const n=i+1;if(n<=state.maxStep)showStep(n);}));$('#submitQuote').addEventListener('click',submitQuote);}
