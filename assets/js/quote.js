...
{id:'paper-glue',cat:'paper',name:'삼면접착 단상자',moq:500,tags:'화장품 · 식품'},
{id:'paper-y1',cat:'paper',name:'Y형 상하 박스',moq:500,tags:'조립형'},
{id:'paper-y2',cat:'paper',name:'Y형 이중고 박스',moq:500,tags:'조립형'},
{id:'paper-sleeve',cat:'paper',name:'Y형 슬리브 박스',moq:500,tags:'슬리브'},
{id:'paper-g',cat:'paper',name:'G형 조립 박스',moq:500,tags:'조립형'},
{id:'paper-handle',cat:'paper',name:'하우스 끈손잡이 박스',moq:500,tags:'손잡이형'},
{id:'paper-window',cat:'paper',name:'오픈 창문형 단상자',moq:500,tags:'제품 노출형'},
{id:'paper-hang',cat:'paper',name:'행잉탭 박스',moq:500,tags:'리테일 진열'},
{id:'paper-donut',cat:'paper',name:'도넛 · 베이커리 박스',moq:500,tags:'식품'},
...
{id:'corr-laminated',cat:'corrugated',name:'골판지 합지박스',moq:500,tags:'칼라박스'},
{id:'corr-gift1',cat:'corrugated',name:'선물박스 · 플라스틱 손잡이',moq:500,tags:'선물포장'},
{id:'corr-gift2',cat:'corrugated',name:'선물박스 · 종이 손잡이',moq:500,tags:'선물포장'},
...
{id:'ccp',name:'CCP',gsm:[300,350,400],note:'유광택 계열'},
...
{id:'earth',name:'얼스팩',gsm:[295],note:'친환경'},
...
const CORR_MATERIALS=[{id:'e',name:'E골',gsm:[],note:'골판지'},{id:'b',name:'B골',gsm:[],note:'골판지'},{id:'e-lam',name:'E골 합지',gsm:[],note:'칼라박스'},{id:'b-lam',name:'B골 합지',gsm:[],note:'칼라박스'}];
...
function boxIcon(cat){const fill=cat==='rigid'?'#0a2240':cat==='corrugated'?'#d9cbb8':cat==='bag'?'#eef1f5':'#fff';return `<svg viewBox="0 0 160 90" width="100%" height="82" aria-hidden="true"><rect x="34" y="23" width="92" height="54" rx="4" fill="${fill}" stroke="#0a2240" stroke-width="1.3"/><path d="M34 23l24-13h91l-23 13M126 23l23-13v54l-23 13" fill="none" stroke="#7f879a" stroke-width="1.3"/></svg>`}
...
function renderPapers(){const list=materials();$('#paperGrid').innerHTML=list.map(p=>`<div class="paper-card ${state.paper?.id===p.id?'selected':''}" data-paper="${p.id}"><div class="paper-swatch"></div><div class="paper-info"><strong>${p.name}</strong><small>${p.note}</small></div></div>`).join('');$$('#paperGrid [data-paper]').forEach(c=>c.onclick=()=>{state.paper=list.find(p=>p.id===c.dataset.paper);state.gsm=null;renderPapers();renderGsm();updateSummary();});}
...
function renderSingle(selector,items,key){$(selector).innerHTML=items.map(v=>`<div class="option-card ${state[key]===v?'selected':''}" data-value="${v}"><strong>${v}</strong></div>`).join('');$$(`${selector} [data-value]`).forEach(c=>c.onclick=()=>{state[key]=c.dataset.value;renderSingle(selector,items,key);updateSummary();});}
function renderFinish(){$('#finishGrid').innerHTML=FINISHES.map(v=>`<div class="option-card ${state.finishes.includes(v)?'selected':''}" data-value="${v}"><strong>${v}</strong></div>`).join('');$$('#finishGrid [data-value]').forEach(c=>c.onclick=()=>{const v=c.dataset.value;state.finishes=state.finishes.includes(v)?state.finishes.filter(x=>x!==v):[...state.finishes,v];renderFinish();updateSummary();});}
...
function showStep(n){state.step=n;state.maxStep=Math.max(state.maxStep,n);$$('.quote-step').forEach(s=>s.classList.toggle('active',Number(s.dataset.step)===n));updateTrack();updateSummary();window.scrollTo({top:$('.quote-process-strip').offsetTop-70,behavior:'smooth'});}
function validateStep(n){captureDimensions();if(n===1){if(!state.product){alert('원하시는 박스 형태를 선택해주세요.');return false}if(!state.qty||(state.qty==='직접입력'&&!state.customQty)){alert('제작 수량을 선택해주세요.');return false}if(!state.w||!state.d||!state.h){alert('완성 사이즈 W / D / H를 입력해주세요.');return false}}if(n===2){if(!state.paper){alert('종이 · 소재를 선택해주세요.');return false}if(state.paper.gsm.length&&!state.gsm){alert('평량을 선택해주세요.');return false}}if(n===3&&!state.printMethod){alert('인쇄 방식을 선택해주세요.');return false}if(n===3&&!state.printColor){alert('인쇄 색상을 선택해주세요.');return false}if(n===3&&!state.printSide){alert('인쇄 면을 선택해주세요.');return false}if(n===4&&!state.coating){alert('코팅 여부를 선택해주세요.');return false}return true}
function submitQuote(){const name=$('#name').value.trim(),phone=$('#phone').value.trim(),email=$('#email').value.trim();if(!name||!phone||!email){alert('담당자명, 연락처, 이메일을 입력해주세요.');return}captureDimensions();const payload={...state,company:$('#company').value.trim(),name,phone,email,message:$('#message').value.trim(),files:[...$('#fileInput').files].map(f=>f.name),createdAt:new Date().toISOString()};localStorage.setItem('fineb_quote_draft',JSON.stringify(payload));alert('제작 사양이 정리되었습니다. 현재는 화면 개발 단계이며 실제 서버 접수는 관리자 시스템 연결 후 활성화됩니다.');}
document.addEventListener('DOMContentLoaded',()=>{const qp=new URLSearchParams(location.search).get('cat');if(CATS.some(([id])=>id===qp))state.cat=qp;renderCats();renderProducts();renderQty();renderPapers();renderGsm();renderSingle('#printMethodGrid',PRINT_METHODS,'printMethod');renderSingle('#printColorGrid',PRINT_COLORS,'printColor');renderSingle('#printSideGrid',PRINT_SIDES,'printSide');renderSingle('#coatingGrid',COATINGS,'coating');renderFinish();renderSingle('#insertGrid',INSERTS,'insert');['width','depth','height'].forEach(id=>$('#'+id).addEventListener('input',updateSummary));$$('.next-step').forEach(btn=>btn.addEventListener('click',()=>{if(validateStep(state.step))showStep(Math.min(5,state.step+1))}));$$('.prev-step').forEach(btn=>btn.addEventListener('click',()=>showStep(Math.max(1,state.step-1))));$$('.track-item').forEach(btn=>btn.addEventListener('click',()=>{const n=Number(btn.dataset.jump);if(n<=state.maxStep)showStep(n)}));$('#submitQuote').addEventListener('click',submitQuote);updateSummary();updateTrack();});