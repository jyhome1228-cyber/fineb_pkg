document.addEventListener('DOMContentLoaded',()=>{
  ...
  const menuLabels=[
    ['about.html','회사소개'],['production.html','제작품목'],['process.html','제작과정'],['works.html','제작사례'],['guide.html','제작가이드'],['sample.html','샘플제작'],['faq.html','자주묻는질문'],['inquiry.html','제작문의'],['quote.html','견적내기 →']
  ];
  ...
  document.querySelectorAll('input[type="file"]').forEach(input=>{
    ...
    if(box&&!box.parentElement?.querySelector('.upload-email-note')){
      const note=document.createElement('div');note.className='upload-email-note';note.innerHTML='첨부파일은 <strong>총 10MB 이하</strong>를 권장합니다. 10MB를 초과하는 파일은 <a href="mailto:whales84@naver.com">whales84@naver.com</a> 으로 보내주세요.';box.insertAdjacentElement('afterend',note);
    }
    input.addEventListener('change',()=>{const total=[...input.files].reduce((sum,file)=>sum+file.size,0);if(total>maxUpload){alert('첨부파일 총 용량이 10MB를 초과합니다. whales84@naver.com 으로 파일을 보내주세요.');input.value='';}});
  });

  function privacyBlock(id){const wrap=document.createElement('div');wrap.className='privacy-consent';wrap.innerHTML=`<label class="privacy-check"><input type="checkbox" id="${id}"><span><strong>[필수] 개인정보 수집 및 이용에 동의합니다.</strong><small>견적·샘플·제작문의 상담 및 회신을 위해 담당자명, 연락처, 이메일, 회사/브랜드명, 요청내용과 첨부파일을 수집합니다.</small></span></label><details><summary>개인정보 수집 및 이용 안내</summary><div>수집 목적: 제작 상담, 견적 안내, 샘플 제작 및 문의 회신<br>수집 항목: 회사/브랜드명, 담당자명, 연락처, 이메일, 제작 사양, 문의내용, 첨부파일<br>보유 기간: 상담 및 제작 업무 종료 후 내부 정책에 따라 보관 후 파기<br>동의를 거부할 수 있으나, 필수 정보 수집에 동의하지 않으면 온라인 문의 접수가 어렵습니다.</div></details>`;return wrap;}
